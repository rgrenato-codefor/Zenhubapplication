...
export default function CompanyCommissions() {
  ...
  const handlePayTherapist = async (e: React.MouseEvent, entry: typeof therapistEarnings[0]) => {
    ...
    // ── Auto-register in financial dashboard ─────────────────────────────────
    addTransaction({
      type: "saida",
      category: "comissoes",
      description: `Comissão – ${entry.therapist.name} (${currentMonth.label})`,
      amount: entry.pendingAmount,
      date: new Date().toISOString().split("T")[0],
      paymentMethod: "Transferência",
      autoGenerated: true,
      notes: `${entry.sessions} sessões · ${entry.rate}% de comissão`,
    });
    setPaying(null);
  };
  ...
  const handlePayAll = async () => {
    ...
    // ── Auto-register one entry per therapist with pending amount ─────────────
    const pendingEntries = therapistEarnings.filter((e) => !e.isPaid && e.pendingAmount > 0);
