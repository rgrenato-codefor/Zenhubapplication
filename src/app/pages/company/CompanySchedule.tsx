...
import {
  ...
  ChevronLeft, ChevronRight, RefreshCw, Info, MapPin, Banknote,
...
export default function CompanySchedule() {
  ...
  const handleConfirmClosure = async () => {
    ...
    // ── Auto-register payment as "entrada" in financial dashboard ─────────
    if (paymentReceived) {
      addTransaction({
        type: "entrada",
        category: "atendimentos",
        description: `Atendimento – ${closureModal.clientName} (${closureModal.therapyName})`,
        amount: total,
        date: apt.date,
        paymentMethod,
        autoGenerated: true,
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className={`rounded-xl border-2 transition-colors overflow-hidden ${paymentReceived ? "border-emerald-200 bg-emerald-50" : "border-gray-100 bg-gray-50"}`}>
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${paymentReceived ? "bg-emerald-100" : "bg-gray-100"}`}>
                      <Banknote className={`w-4 h-4 transition-colors ${paymentReceived ? "text-emerald-600" : "text-gray-400"}`} />
                    </div>
                    <div className="text-left">
                      <p className={`text-sm transition-colors ${paymentReceived ? "text-emerald-800" : "text-gray-600"}`} style={{ fontWeight: 600 }}>
                        Pagamento recebido
                      </p>
                      <p className={`text-xs transition-colors ${paymentReceived ? "text-emerald-600" : "text-gray-400"}`}>
                        {paymentReceived ? "Será lançado como entrada no financeiro" : "Registrar pagamento depois"}
                      </p>
                    </div>
                  </div>
                  {/* Toggle pill */}
                  <div className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${paymentReceived ? "bg-emerald-500" : "bg-gray-300"}`}>
                    <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${paymentReceived ? "translate-x-5" : "translate-x-0.5"}`} />
                  </div>
                </button>

                {paymentReceived && (
                  <div className="px-4 pb-3 border-t border-emerald-100">
                    <p className="text-xs text-emerald-700 mt-2 mb-2" style={{ fontWeight: 600 }}>Forma de pagamento</p>
                    <div className="flex flex-wrap gap-2">
                      {["PIX", "Dinheiro", "Cartão de Crédito", "Cartão de Débito", "Transferência"].map((m) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs transition-all ${paymentMethod === m ? "text-white" : "bg-white border border-emerald-200 text-emerald-700 hover:bg-emerald-50"}`}
                          style={paymentMethod === m ? { background: "#059669", fontWeight: 700 } : { fontWeight: 500 }}
                        >
                          {m}
                        </button>
                      ))}
