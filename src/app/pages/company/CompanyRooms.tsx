...
// ── Constants (module-level to avoid recreation on every render) ──────────────
const HOURS = [
...
// ── RoomModal (defined OUTSIDE CompanyRooms to avoid remount on every render) ─
function RoomModal({
  mode,
  initial,
  companyId,
  ...
  const handleSave = () => {
    ...
    onSave({ id: initial?.id ?? `r_${Date.now()}`, companyId, ...form });
  };
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Ex: Espaço para massagens relaxantes..."
          ...
          {companyUnits.length > 0 && (
            <div>
              <label className="block text-xs text-gray-500 mb-1.5" style={{ fontWeight: 600 }}>Unidade</label>
              <select
                value={form.unitId ?? ""}
          ...
          {/* Color */}
          <div>
            <label className="block text-xs text-gray-500 mb-2" style={{ fontWeight: 600 }}>Cor de identificação</label>
          ...
          {/* Status */}
          <div>
            <label className="block text-xs text-gray-500 mb-1.5" style={{ fontWeight: 600 }}>Status</label>
...
export default function CompanyRooms() {
  ...
  const { user } = useAuth();
  const {
    company, appointments, rooms: firestoreRooms, roomStore: store,
  ...
  const handleSave = async (room: Room) => {
    if (modal?.mode === "add") {
      ...
      await mutateUpdateRoom(room.id, {
        name: room.name, description: room.description,
        color: room.color, status: room.status,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {activeCount} {activeCount === 1 ? "sala ativa" : "salas ativas"} · {availableNow} disponíveis hoje
          </p>
        </div>
        <div className="flex items-center gap-3">
          {toast && (
          ...
          <div className="flex bg-gray-100 rounded-lg p-0.5">
            ...
            >
              <LayoutGrid className="w-3.5 h-3.5" /> Salas
            </button>
            <button
            ...
            >
              <CalendarDays className="w-3.5 h-3.5" /> Disponibilidade
            </button>
          </div>
          <button
          ...
          {filteredRooms.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
              ...
              <p className="text-gray-400 text-sm">
                {companyRooms.length === 0 ? "Nenhuma sala cadastrada ainda." : "Nenhuma sala encontrada com esse filtro."}
              </p>
                  ...
                  <div key={room.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ background: room.color }}>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => setModal({ mode: "edit", room })} className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
                            <Edit2 className="w-3.5 h-3.5 text-gray-500" />
                          </button>
                          <button onClick={() => setDeleteConfirm(room)} className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-red-100 flex items-center justify-center transition-colors">
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        <span className="flex items-center gap-1 text-xs px-2 py-1 rounded-full" style={{ background: sc.bg, color: sc.color, fontWeight: 600 }}>
                          <StatusIcon status={room.status} />
                          {sc.label}
                        </span>
                        {room.status === "active" && (
                          <span className="text-xs px-2 py-1 rounded-full" style={isOccupied ? { background: "#FEF2F2", color: "#DC2626", fontWeight: 600 } : { background: "#ECFDF5", color: "#059669", fontWeight: 600 }}>
                          ...
                          {todayApts.slice(0, 3).map((apt) => {
                            ...
                            return (
                              <div key={apt.id} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50">
                                ...
                                <span className="text-xs text-gray-400 ml-auto shrink-0">{therapist?.name.split(" ")[0]}</span>
                              </div>
                            );
                          })}
                          {todayApts.length > 3 && <p className="text-xs text-gray-400 text-center">+{todayApts.length - 3} mais</p>}
                        </div>
                      ) : (
                        room.status === "active" && (
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-lg" style={{ background: `${room.color}15`, color: room.color, fontWeight: 600 }}>
            ...
            {DAYS_WEEK.map((day) => (
              <button
                ...
                className={`rounded-xl p-2.5 text-center border transition-all ${
                  selectedDay === day.date ? "text-white shadow-md" : day.isToday ? "bg-white border-gray-300 shadow-sm" : "bg-white border-gray-100"
                }`}
                style={selectedDay === day.date ? { background: primaryColor, borderColor: primaryColor } : {}}
              >
              ...
              <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-4 ml-auto">
                  <span className="flex items-center gap-1.5 text-xs text-gray-500">
                    <span className="w-3 h-3 rounded-sm bg-gray-100 inline-block" /> Livre
                  </span>
                  <span className="flex items-center gap-1.5 text-xs text-gray-500">
                    <span className="w-3 h-3 rounded-sm bg-violet-200 inline-block" /> Ocupada
                  </span>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {HOURS.map((hour) => (
                      <tr key={hour} className="hover:bg-gray-50/50">
                        <td className="px-4 py-2 text-xs text-gray-400 text-right border-r border-gray-100">{hour}</td>
                        {activeRooms.map((room) => {
                            ...
                            <td key={room.id} className="px-2 py-1.5 text-center">
                              {apt ? (
                                ...
                                >
                                  {isStart ? (
                                    <>
                                      <p className="text-xs truncate" style={{ color: room.color, fontWeight: 700 }}>{aptClient?.name.split(" ")[0]}</p>
                                      <div className="flex items-center gap-1 mt-0.5">
                                        <User className="w-2.5 h-2.5 text-gray-400" />
                                        <p className="text-xs text-gray-500 truncate">{aptTherapist?.name.split(" ")[0]}</p>
                                      </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-gray-900 text-center mb-1" style={{ fontWeight: 700 }}>Remover sala?</h3>
            <p className="text-gray-500 text-sm text-center mb-5">
              A sala <strong>{deleteConfirm.name}</strong> será removida. Os agendamentos existentes perderão a associação com essa sala.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 text-sm">
                Cancelar
              </button>
              <button onClick={() => handleDelete(deleteConfirm)} className="flex-1 py-2.5 rounded-xl bg-red-500 text-white text-sm" style={{ fontWeight: 700 }}>
                Remover
              </button>
            </div>
          </div>
