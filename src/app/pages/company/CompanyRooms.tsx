...
import { useRoomStore, type Room, type RoomStatus } from "../../store/roomStore";
import { appointments, therapists, clients, therapies, companies, units } from "../../data/mockData";
...
export default function CompanyRooms() {
  ...
  const rooms = roomStore.getRooms(companyId);
  const companyApts = appointments.filter((a) => a.companyId === companyId);
  ...
  const todayOccupied = new Set(
    companyApts
      ...
      .map((a) => roomStore.getRoomForAppointment(a.id))
      .filter(Boolean)
  );
  const availableNow = rooms.filter(
    (r) => r.status === "active" && !todayOccupied.has(r.id)
  ).length;

  // Availability grid helpers
  const getAptForRoomHour = (roomId: string, hour: string) => {
    return companyApts.find((a) => {
      const assignedRoom = roomStore.getRoomForAppointment(a.id);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Total de salas", value: rooms.length, color: "text-gray-900", bg: "bg-gray-50", icon: DoorOpen },
          ...
          {filteredRooms.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredRooms.map((room) => {
                const todayApts = companyApts.filter(
                  (a) => roomStore.getRoomForAppointment(a.id) === room.id && a.date === "2026-03-04"
                );
                const weekApts = companyApts.filter(
                  (a) => roomStore.getRoomForAppointment(a.id) === room.id
                );
